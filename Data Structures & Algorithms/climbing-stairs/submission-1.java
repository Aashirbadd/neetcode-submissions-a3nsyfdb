class Solution {
    ...
    public int dfs(int curr, int goal, int currStepsTaken, HashMap<Integer, Integer> cache) {
        ...
        // The increment cases
        int oneStep = dfs(curr + 1, goal, currStepsTaken + 1, cache);
        cache.put(curr + 1, oneStep);
        int twoStep = dfs(curr + 2, goal, currStepsTaken + 1, cache);
        cache.put(curr + 2, twoStep);
